...
using HecopUI_Winforms.Controls.Charts.Model;
...
namespace HecopUI_Winforms.Controls.Charts
{
...
#pragma warning restore IDE0052 // Remove unread private members
        ...
        HecopUI_Winforms.Controls.Charts.Model.DataItems dataItem = new DataItems();
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            try
            {
                ...
                using (var BrOyNumber = new SolidBrush(Color.Black))
                using (var BrGrid= new SolidBrush(gridColor))
                {
                    ...
                    g.DrawLine(new Pen(Color.Black, 1), new PointF(offX - 5, Height - offch), new PointF(Width - offcw + 8, Height - offch)); // Oy
                    g.DrawLine(new Pen(Color.Black, 1), new PointF(offX, offY - 5), new PointF(offX, Height - offch + 5)); //Ox
                    ...
                    for (int item = 0; item < dataItem.Items.Count; item++)
                    {
                        ...
                        if (dataIte != null)
                        {
                            ...
                            // Hiện tooltip khi di chuột vào các điểm
                            //for (int i = 0; i < points.Count; i++)
                            //{
                            //    var re = new RectangleF(points[i].X - pointSize / 2, points[i].Y - pointSize / 2, pointSize, pointSize);
                            //    Point mp = PointToClient(MousePosition);

                            //    if (re.Contains(mp) && hover)
                            //    {
                            //        // Lấy giá trị trực tiếp từ dữ liệu của mục hiện tại
                            //        var key = dataItem.Items.ElementAt(item).Data.ElementAt(i).Key;
                            //        dataItem.Items.ElementAt(item).Data.TryGetValue(key, out int value);
                            //        // Hiển thị tooltip tại vị trí chuột
                            //        tooltipText = $"{key}: {value}";

                            //        ttsi = e.Graphics.MeasureString(tooltipText, Font);
                            //        ttp = new PointF(mp.X - 5 - ttsi.Width / 2, mp.Y - pointSize - ttsi.Height - 5);
                    ...
                    //if (!DesignMode && hover && tooltipText != "")
                    //{
                    //    g.SmoothingMode = SmoothingMode.Default;
                    //    using(Pen pen = new Pen(Color.DimGray, 1))
                    //    using (SolidBrush brush = new SolidBrush(Color.White))
                    //    {
                    //        g.FillRectangle(brush, ttp.X, ttp.Y, ttsi.Width + 4, ttsi.Height + 4);
                    //        g.DrawRectangle(pen, ttp.X, ttp.Y, ttsi.Width + 4, ttsi.Height + 4);
                    //        g.DrawString(tooltipText, Font, pen.Brush, ttp.X + 2, ttp.Y + 2);
                    //    }
                    //}
