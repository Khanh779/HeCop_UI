...
namespace HecopUI_Winforms.Controls
{
    public partial class HDotProgressRing : Control
    {
        ...
        private void V_Tick(object sender, EventArgs e)
        {
            ...
            else if (ProgressStyle == Style.Style5)
            {
        ...
        public enum Style { Style1, Style2, Style3, Style4, Style5, Style6 }
        ...
        int next = 0;
        Color dotColor = GetAppResources.GetColorNormal();
        ...
        {
            using (Brush rippleBrush = new SolidBrush(Color.FromArgb((int)(130 - (120 * _value / dotCount)), DotColor)))
            {
                var rippleSize = (int)(Math.Max(Width, Height) * _value / dotCount) - 2;
                e.Graphics.FillEllipse(rippleBrush, new Rectangle(Width / 2 - rippleSize / 2, Height / 2 - rippleSize / 2, rippleSize, rippleSize));
            }
        }
