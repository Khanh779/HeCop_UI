            ...
            for (var yy = 0; yy < image.Height; yy++)
            {
                ...
                for (var x = xx; x < xx + blurSize && x < image.Width && x < blurred.Width; x++)
                    for (var y = yy; y < yy + blurSize && y < image.Height && y < blurred.Height; y++)
                        blurred.SetPixel(x, y, Color.FromArgb((int)avgR, (int)avgG, (int)avgB));
...
public partial class ImageBlur
{
            ...
            for (int x = radius; x < width - radius; x++)
            {
                ...
                for (int j = -radius; j <= radius; j++)
                {
                    for (int i = -radius; i <= radius; i++)
                    {
                        ...
                        double factor = kernel[i + radius, j + radius];
                ...
                Color blurredColor = Color.FromArgb(
                    (int)Math.Round(a),
                    (int)Math.Round(r),
                    (int)Math.Round(g),
                    (int)Math.Round(b)
    ...
    public static Bitmap BoxBlur(Bitmap image, int radius)
    {
        if (radius < 1)
        {
            throw new ArgumentException("Radius must be greater than zero.");
        ...
        int width = image.Width;
        int height = image.Height;
        double[,] kernel = CreateBoxKernel(radius);
    ...
    private static double[,] CreateBoxKernel(int radius)
    {
        int size = 2 * radius + 1;
