...
namespace HecopUI_Winforms.Controls
{
    ...
    public partial class HSolidGauge : Control
    {
        ...
        private Color _GaugeColor1 = GetAppResources.GaugeBarColor();
        private Color _GaugeColor2 = GetAppResources.GaugeBarColor();
        ...
        private Color BG = GetAppResources.BaseGaugeColor();
            ...
            using (Bitmap bitmap = new Bitmap(this.Width, this.Height, System.Drawing.Imaging.PixelFormat.Format64bppPArgb))
            {
                using (Graphics graphics = Graphics.FromImage(bitmap))
                {
                    ...
                    if (gaugeMode == GaugeType.Transition)
                    {
                        ...
                        brush = new SolidBrush(DrawHelper.BlendColor(GaugeColor1, GaugeColor2, blend));
                    }
                    ...
                    //Single minAngle = (int)(m_BaseArcStart + (0) * m_BaseArcSweep / (100 - 0)) % 360;
                    //if (minAngle < 0) minAngle += 360;
                    //Double aneedleAngle = minAngle * Math.PI / 180;
                    //Point minPoint = new Point((int)(Width / 2 + (Width / 2) * Math.Cos(aneedleAngle)), (int)(Height / 2 + (Height / 2) * Math.Sin(aneedleAngle)));
                    //Single maxAngle = (int)(m_BaseArcStart + (100) * m_BaseArcSweep / (100 - 0)) % 360;
                    //if (maxAngle < 0) maxAngle += 360;
                    //Double bneedleAngle = maxAngle * Math.PI / 180;
                    //Point maxPoint = new Point((int)(Width / 2 + (Width / 2) * Math.Cos(bneedleAngle)), (int)(Height / 2 + (Height / 2) * Math.Sin(bneedleAngle)));
                    //if (ShowMajor == true)
                    //{
                    //    //SizeF minRe = graphics.MeasureString(m_MinValue.ToString(), Font);
                    //    e.Graphics.DrawString(m_MinValue.ToString(), Font, new SolidBrush(ForeColor), minPoint.X, minPoint.Y + SolidGaugeWidth / 3 - 5);
                    //    e.Graphics.DrawString(m_MaxValue.ToString(), Font, new SolidBrush(ForeColor), maxPoint.X - 15, maxPoint.Y + SolidGaugeWidth / 3 - 5);
                    //}
                    ...
                    for (int i = 0; i <= numberOfDivisions; i++)
                    {
                        ...
                        {
                            //SizeF minRe = graphics.MeasureString(m_MinValue.ToString(), Font);
                            e.Graphics.DrawString((i * 10).ToString(), Font, new SolidBrush(ForeColor), minPoint.X- SolidGaugeWidth/2-2, minPoint.Y +SolidGaugeWidth/2- 12);
                        }
                    }
        ...
        private Color needleColor = GetAppResources.GetColorNormal();
