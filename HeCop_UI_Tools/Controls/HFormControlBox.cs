...
namespace HecopUI_Winforms.Controls
{
    ...
    public partial class HFormControlBox : Control
    {
        ...
        private void HFormControlBox1_Paint(object sender, PaintEventArgs e)
        {
            ...
            SolidBrush SB = new SolidBrush(hovered ? DrawHelper.BlendColor(ControlBoxColor, ControlBoxHoverColor, step) : ControlBoxColor);
            switch (HoverColorType)
            {
                ...
                case ShapeType.Rectangle:
                    g.FillPath(SB, DrawHelper.GetRoundPath(ClientRectangle, rad));
                    break;
            }
        ...
        {
            float penw = 2f;
            System.Drawing.Pen pen = new System.Drawing.Pen(new SolidBrush(DrawHelper.BlendColor(IconColor, IconHoverColor, step)), penw) { Alignment = PenAlignment.Inset };
            float offs = penw - 1;
            switch (iT)
            {
                case IconType.Hide:
                ...
                case IconType.NormalAndMaximize:
                    ...
                    else
                    {
                        ...
                        //g.DrawPath(pen, DrawHelper.GetRoundPath(new RectangleF(rect.X,rect.Y,rect.Width-1,rect.Height-1), 3,3));
                    }
                    break;
                case IconType.Minimize:
