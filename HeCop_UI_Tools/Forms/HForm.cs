...
namespace HecopUI_Winforms.Forms
{
    public partial class HForm : Form
    {
        ...
        private Color borderColor = GetAppResources.GetBorderColor();
        /// <summary>
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            {
                HecopUI_Winforms.GetAppResources.GetControlGraphicsEffect(g);
                ...
                {
                    int sc = 12;
                    using (SolidBrush closeBrush = new SolidBrush(DrawHelper.BlendColor(CB.CloseBoxColor, CB.CloseBoxHoverColor, closestep)))
                        g.FillPath(closeBrush, (CB.HoverColorShape == ShapeType.Rectangle) ? DrawHelper.GetRoundPath(ControlBoxRect, 5) : DrawHelper.GetRoundPath(ControlBoxRect, sc));

                    if (MaximizeBox)
                        using (SolidBrush maximizeBrush = new SolidBrush(DrawHelper.BlendColor(CB.MaximizeBoxColor, CB.MaximizeBoxHoverColor, maxstep)))
                            g.FillPath(maximizeBrush, (CB.HoverColorShape == ShapeType.Rectangle) ? DrawHelper.GetRoundPath(MaximizeBoxRect, 5) : DrawHelper.GetRoundPath(MaximizeBoxRect, sc));

                    if (MinimizeBox)
                        using (SolidBrush minimizeBrush = new SolidBrush(DrawHelper.BlendColor(CB.MinimizeBoxColor, CB.MinimizeBoxHoverColor, minstep)))
                            g.FillPath(minimizeBrush, (CB.HoverColorShape == ShapeType.Rectangle) ? DrawHelper.GetRoundPath(MinimizeBoxRect, 5) : DrawHelper.GetRoundPath(MinimizeBoxRect, sc));
                    ...
                    //if (MaximizeBox == false)
                    using (Pen minimizePen = new Pen(new SolidBrush(DrawHelper.BlendColor(CB.IconMinimizeColor, CB.IconMinimizeHoverColor, minstep)), 1.5f))
