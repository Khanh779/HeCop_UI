...
namespace HecopUI_Winforms.Controls
{
    ...
    public partial class HCheckedButton : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (ClipRegion == true) if (DesignMode == false)
                {
                    ...
                    if (ST == ShapeType.RoundRectangle)
                        Region = new Region(DrawHelper.GetRoundPath(new RectangleF(0, 0, Width, Height), Rad - 2.5f));
            ...
            LinearGradientBrush GHB = new LinearGradientBrush(ClientRectangle, Checked ? ButtonCheckedColor1 : ButDo ? ButtonDownColor1 : ButHo ? ButtonHoverColor1 : ButtonColor1, Checked ? ButtonCheckedColor2 : ButDo ? ButtonDownColor2 : ButHo ? ButtonHoverColor2 : ButtonColor2, LB);
            LinearGradientBrush AHB = new LinearGradientBrush(ClientRectangle, Checked ? ButtonCheckedColor1 : ButHo ? ButtonHoverColor1 : ButtonColor1, Checked ? ButtonCheckedColor2 : ButHo ? ButtonHoverColor2 : ButtonColor2, LB);
            StringFormat SF = new StringFormat(); int b = 0;
            GraphicsPath Gp = DrawHelper.GetRoundPath(new RectangleF(b + (shadowPadding.Left), b + (shadowPadding.Top), (Width - b - shadowPadding.Left) - (shadowPadding.Right), (Height - b - shadowPadding.Top) - (shadowPadding.Bottom)), Rad, BorderThickness);
            GraphicsPath SGP = DrawHelper.GetRoundPath(new RectangleF(b, b, Width - b, Height - b), Rad);
            GraphicsPath FillPa = DrawHelper.GetRoundPath(new RectangleF(b + (shadowPadding.Left), b + (shadowPadding.Top), (Width - b - shadowPadding.Left) - (shadowPadding.Right), (Height - b - shadowPadding.Top) - (shadowPadding.Bottom)), Rad);
            ...
            {
                AHB = new LinearGradientBrush(ClientRectangle, Checked ? ButtonCheckedColor1 : ButDo ? ButtonDownColor1 : ButHo ? DrawHelper.BlendColor(ButtonColor1, ButtonHoverColor1, _animationManager.GetProgress()) : DrawHelper.BlendColor(ButtonColor1, ButtonHoverColor1, _animationManager.GetProgress()), Checked ? ButtonCheckedColor2 : ButDo ? ButtonDownColor2 : ButHo ? DrawHelper.BlendColor(ButtonColor2, ButtonHoverColor2, _animationManager.GetProgress()) : DrawHelper.BlendColor(ButtonColor2, ButtonHoverColor2, _animationManager.GetProgress()), LB);
            }
            pen.Alignment = PenAlignment.Inset;
            Bitmap Shado = HecopUI_Winforms.Ultils.DropShadow.Create(SGP, ShadowColor, shadowRad);
            ...
            if (DesignMode == false)
                if (foc)
                {
                    GraphicsPath gpf = DrawHelper.GetRoundPath(new RectangleF(b + (shadowPadding.Left), b + (shadowPadding.Top),
